...
type Context = {
  params: Promise<{ documentKind: string; documentId: string }>
}
...
export async function GET(req: NextRequest, context: Context) {
  try {
    ...
    const archiveKind = parseDocumentArchiveKind(
      req.nextUrl.searchParams.get("archiveKind") ?? "DOCUMENT_PDF"
    )
    ...
    if (!row) {
      throw new DocumentArchiveError(
        "Active document archive not found for document reference",
    ...
    return documentArchiveApiErrorResponse(
      err,
      "GET /api/document-archive/by-document/[documentKind]/[documentId]/file"
    )
  }
}

...
import { confirmPettyCashVoucher } from "@/lib/finance/petty-cash-voucher/petty-cash-voucher-workflow"
...
export async function POST(_req: NextRequest, context: Context) {
  try {
    ...
    await confirmPettyCashVoucher({ entryId: id, confirmedByStaffId: actor.staffId })
    const entry = await getPettyCashVoucherById(prisma, id)
    return NextResponse.json({ entry })
  } catch (err: unknown) {
    return pettyCashVoucherErrorResponse(err, "POST petty-cash-vouchers/[id]/confirm")
  }
}

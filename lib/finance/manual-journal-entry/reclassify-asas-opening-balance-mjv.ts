...
export const ASAS_OPENING_BALANCE_RECLASSIFY_TARGET = {
  entryNo: "MJV-260001",
  legalEntityCode: "AS",
...
export const ASAS_OPENING_BALANCE_RECLASSIFY_CONFIRM_TOKEN =
  "ASAS_OPB_RECLASSIFY_CONFIRMED"

export class ReclassifyAsasOpeningBalanceError extends Error {
  readonly code: string

  constructor(message: string, code: string) {
    super(message)
    this.name = "ReclassifyAsasOpeningBalanceError"
    this.code = code
  }
}

export type ReclassifyAsasOpeningBalanceSnapshot = {
...
export type ReclassifyAsasOpeningBalancePlan = {
  before: ReclassifyAsasOpeningBalanceSnapshot
  after: ReclassifyAsasOpeningBalanceSnapshot
  unchanged: boolean
...
function buildSnapshot(row: ReclassifyTargetRow): ReclassifyAsasOpeningBalanceSnapshot {
...
function buildAfterSnapshot(
  before: ReclassifyAsasOpeningBalanceSnapshot
): ReclassifyAsasOpeningBalanceSnapshot {
  return {
    ...before,
...
async function loadReclassifyTarget(
  ...
  const { entryNo, legalEntityCode, description } = ASAS_OPENING_BALANCE_RECLASSIFY_TARGET

  const matches = await tx.manualJournalEntry.findMany({
    where: { entryNo, legalEntityCode, description },
    include: {
  ...
  const { targetPeriodKey, targetEntryDate } = ASAS_OPENING_BALANCE_RECLASSIFY_TARGET
...
export async function planReclassifyAsasOpeningBalanceMjv(
  tx: Prisma.TransactionClient = prisma
): Promise<ReclassifyAsasOpeningBalancePlan> {
...
export async function executeReclassifyAsasOpeningBalanceMjv(): Promise<ReclassifyAsasOpeningBalancePlan> {
  return prisma.$transaction(async (innerTx) => {
    const plan = await planReclassifyAsasOpeningBalanceMjv(innerTx)
    if (plan.unchanged) {
      return plan
    }
    ...
    return planReclassifyAsasOpeningBalanceMjv(innerTx)
  })
}
...
export async function assertOpeningBalanceReviewQueryFindsRow(
  ...
  if (!row) {
    throw new ReclassifyAsasOpeningBalanceError(
      "Opening Balance Review query did not find OPENING_BALANCE journal after reclassify",
...
export async function verifyOpeningBalanceReviewAfterReclassify(
  ...
  if (failed.length > 0) {
    throw new ReclassifyAsasOpeningBalanceError(
      `Opening Balance Review checks failed after reclassify: ${failed.join(", ")}`,
...
export function formatReclassifyAsasOpeningBalanceSnapshot(
  snapshot: ReclassifyAsasOpeningBalanceSnapshot,
  label: "before" | "after"
): string {
  return [
    `[${label}] ASAS opening balance reclassify snapshot`,

...
export type PettyCashVoucherReadPrisma = Pick<PrismaClient, "pettyCashVoucher">
...
type EntryDetail = NonNullable<
  Awaited<ReturnType<PettyCashVoucherReadPrisma["pettyCashVoucher"]["findFirst"]>>
> & {
  pettyCashAccount: { code: string; name: string }
  lines: Array<{
    id: string
    lineNo: number
    glAccountId: string
...
function mapLine(line: EntryDetail["lines"][number]): PettyCashVoucherLineRead {
  return {
    id: line.id,
    lineNo: line.lineNo,
...
function mapEntry(entry: EntryDetail): PettyCashVoucherRead {
  ...
  return {
    ...
    pettyCashAccountId: entry.pettyCashAccountId,
    pettyCashAccountCode: entry.pettyCashAccount.code,
    pettyCashAccountName: entry.pettyCashAccount.name,
...
export async function listPettyCashVouchers(
  db: PettyCashVoucherReadPrisma,
  filter: PettyCashVoucherListFilter = {}
  ...
  const [rows, total] = await Promise.all([
    ...
    db.pettyCashVoucher.count({ where }),
  ])

  const entries: PettyCashVoucherListItem[] = rows.map((row) => ({
    id: row.id,
    entryNo: row.entryNo,
    status: row.status,

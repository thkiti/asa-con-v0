...
export type PaymentVoucherReadPrisma = Pick<PrismaClient, "paymentVoucher">
...
type EntryDetail = NonNullable<
  Awaited<ReturnType<PaymentVoucherReadPrisma["paymentVoucher"]["findFirst"]>>
> & {
  payFromAccount: { code: string; name: string }
  lines: Array<{
    id: string
    lineNo: number
    glAccountId: string
...
function mapLine(line: EntryDetail["lines"][number]): PaymentVoucherLineRead {
  return {
    id: line.id,
    lineNo: line.lineNo,
...
function mapEntry(entry: EntryDetail): PaymentVoucherRead {
  ...
  return {
    ...
    payFromAccountId: entry.payFromAccountId,
    payFromAccountCode: entry.payFromAccount.code,
    payFromAccountName: entry.payFromAccount.name,
...
export async function listPaymentVouchers(
  db: PaymentVoucherReadPrisma,
  filter: PaymentVoucherListFilter = {}
  ...
  const entries: PaymentVoucherListItem[] = rows.map((row) => ({
    id: row.id,
    entryNo: row.entryNo,
    status: row.status,

import type { PaymentEvidenceMobileMeta } from "@/lib/pos/payment-evidence-upload-token"
...
const PAGE_STYLE = `
  ...
  .btn { display: block; width: 100%; margin-top: 1rem; padding: 1rem; font-size: 1rem; font-weight: 700;
    border: none; border-radius: 0.75rem; background: #65a30d; color: #fff; cursor: pointer; }
...
export function buildMobileUploadPagePath(token: string): string {
  return `/payment-evidence/mobile/${encodeURIComponent(token)}`
}
...
export function renderMobileUploadFormPage(input: {
  token: string
  meta: PaymentEvidenceMobileMeta
}): string {
  const shop = `${input.meta.branchCode}${
    input.meta.branchName ? ` — ${input.meta.branchName}` : ""
  }`
  const amount = Number(input.meta.amount).toLocaleString("en-US", {
  ...
  return pageShell(
    ...
    `
    <h1>Bank Transfer Slip Upload</h1>
    <p>Upload the transfer slip for this receipt.</p>
    <section class="card">
      <div class="row"><span class="label">Receipt No</span><span class="mono">${escapeHtml(input.meta.receiptNo)}</span></div>
      <div class="row"><span class="label">Shop</span><span>${escapeHtml(shop)}</span></div>
      <div class="row"><span class="label">Amount</span><span>${escapeHtml(amount)}</span></div>
    </section>
    <section class="upload-section">
      <h2>Take Photo</h2>
      <form action="/api/payment-evidence/mobile/upload" method="post" enctype="multipart/form-data">
...
export function renderMobileUploadSuccessPage(): string {
  return pageShell(
    "Upload Complete",
    `
...
export function renderMobileUploadErrorPage(input: {
  title?: string
  message: string
...
export function isHtmlFormUpload(form: FormData): boolean {

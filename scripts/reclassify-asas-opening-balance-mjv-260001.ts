/**
 * One-time ops: reclassify posted ASAS MJV-260001 as OPENING_BALANCE.
 *
 * Run only after successful UI post of AS MJV-260001.
 * Aligns document identity only — no line/amount/date/period/PDF changes.
 *
 * Dry run (default):
 *   npx tsx scripts/reclassify-asas-opening-balance-mjv-260001.ts
 *
 * Execute (requires env guard + confirmation token):
 *   FINANCE_OPENING_BALANCE_RECLASSIFY_ENABLED=true npx tsx scripts/reclassify-asas-opening-balance-mjv-260001.ts --execute --confirm=ASAS_OPB_RECLASSIFY_CONFIRMED
 */
...
import {
  ASAS_OPENING_BALANCE_RECLASSIFY_CONFIRM_TOKEN,
  ASAS_OPENING_BALANCE_RECLASSIFY_TARGET,
  assertOpeningBalanceReviewQueryFindsRow,
  executeReclassifyAsasOpeningBalanceMjv,
  formatReclassifyAsasOpeningBalanceSnapshot,
  planReclassifyAsasOpeningBalanceMjv,
  ReclassifyAsasOpeningBalanceError,
  verifyOpeningBalanceReviewAfterReclassify,
} from "@/lib/finance/manual-journal-entry/reclassify-asas-opening-balance-mjv"
...
function validateExecute(input: { execute: boolean; confirm: string | null }, dbUrl: string) {
  ...
  if (input.confirm !== ASAS_OPENING_BALANCE_RECLASSIFY_CONFIRM_TOKEN) {
    throw new ReclassifyAsasOpeningBalanceError(
      `Execute requires --confirm=${ASAS_OPENING_BALANCE_RECLASSIFY_CONFIRM_TOKEN}`,
  ...
  if (!dbTarget.isLocalhost && input.confirm !== ASAS_OPENING_BALANCE_RECLASSIFY_CONFIRM_TOKEN) {
    throw new ReclassifyAsasOpeningBalanceError(
...
async function main() {
  ...
  console.log("=== ASAS Opening Balance Reclassify (MJV-260001) ===")
  console.log("Mode:", cli.execute ? "EXECUTE" : "DRY RUN")
  console.log(
    "Target:",
    `${ASAS_OPENING_BALANCE_RECLASSIFY_TARGET.legalEntityCode} / ${ASAS_OPENING_BALANCE_RECLASSIFY_TARGET.entryNo}`
  )
  console.log("Period:", ASAS_OPENING_BALANCE_RECLASSIFY_TARGET.targetPeriodKey)
  console.log(
    "Change:",
  ...
  try {
    const planned = await planReclassifyAsasOpeningBalanceMjv()
    console.log("\n--- Before ---")
    console.log(formatReclassifyAsasOpeningBalanceSnapshot(planned.before, "before"))
    console.log("\n--- After (planned) ---")
    console.log(formatReclassifyAsasOpeningBalanceSnapshot(planned.after, "after"))
    ...
    if (planned.unchanged) {
      ...
      const lookup = await assertOpeningBalanceReviewQueryFindsRow(prisma, {
        legalEntityCode: ASAS_OPENING_BALANCE_RECLASSIFY_TARGET.legalEntityCode,
        periodKey: ASAS_OPENING_BALANCE_RECLASSIFY_TARGET.targetPeriodKey,
      })
    ...
    if (!cli.execute) {
      ...
      console.log(
        "To execute after UI post and backup:\n  FINANCE_OPENING_BALANCE_RECLASSIFY_ENABLED=true npx tsx scripts/reclassify-asas-opening-balance-mjv-260001.ts",
        `--execute --confirm=${ASAS_OPENING_BALANCE_RECLASSIFY_CONFIRM_TOKEN}`
      )
      return
    }

    console.log("\nApplying reclassify in transaction…")
    const applied = await executeReclassifyAsasOpeningBalanceMjv()
    console.log("\n--- After (applied) ---")
    console.log(formatReclassifyAsasOpeningBalanceSnapshot(applied.after, "after"))
    ...
    if (err instanceof ReclassifyAsasOpeningBalanceError) {

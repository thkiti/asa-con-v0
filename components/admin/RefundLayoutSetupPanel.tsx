...
type RefundLayoutSetupPanelProps = {
...
export function RefundLayoutSetupPanel({
  initialLayout,
  receiptLayout,
  onSaved,
}: RefundLayoutSetupPanelProps) {
  ...
  const sampleRefund = useMemo(() => {
    if (!selectedBranch) return null
    return buildRefundSetupSampleContext({
      branch: selectedBranch,
      companyTaxId,
    })
  }, [selectedBranch, companyTaxId])

  const ticketLayout = useMemo(() => {
    if (!sampleRefund) return null
    return buildRefundSetupTicketLayout({
      receiptLayout,
      refundLayout: draft,
      refund: sampleRefund,
    })
  }, [sampleRefund, receiptLayout, draft])

  const printSampleSlip = useMemo(() => {
    if (!ticketLayout) return null
    return <RefundSetupPreview layout={ticketLayout} />
  }, [ticketLayout])
  ...
  const onSave = useCallback(async () => {
    ...
    setSaved(false)
    try {
      const result = await patchThermalDocumentLayout("REFUND", blockLayoutInputFromDraft(draft))
  ...
  return (
    <div className="mt-6 grid gap-6 lg:grid-cols-2">
      <div className={`${themeCard} space-y-3 p-4`}>
        <h2 className={themePageTitle}>Refund layout</h2>
        <p className={`text-xs ${themeMuted}`}>
          Header, sub-header, and footer for refund slips. Empty blocks inherit from Receipt when
          printing. Refund body and Phone No / Sign stay refund-specific.
        </p>
      ...
      <div className={`${themeCard} p-4`}>
        ...
        <p className={`mt-1 text-xs ${themeMuted}`}>
          80mm paper · 72mm printable · sample refund slip using live shop data.
        </p>
        ...
        <label className="mt-3 block">
          ...
          <select
            ...
            data-testid="refund-setup-shop-select"
          >
            {branches.map((branch) => (
        ...
        <div className="receipt-setup-preview mt-3">
          {branchesLoading ? (
            ...
            <RefundSetupPreview layout={ticketLayout} />
          ) : (

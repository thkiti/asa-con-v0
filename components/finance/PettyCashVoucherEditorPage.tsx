...
import {
  ...
  parsePettyCashVoucherAmount,
  PETTY_CASH_VOUCHER_ENTRY_TYPE,
...
function editorSeed(initialEntry: PettyCashVoucherRead | null) {
  if (!initialEntry) {
    return {
      entry: null as PettyCashVoucherRead | null,
      branchId: "",
  ...
  return {
    ...
    pettyCashAccountId: initialEntry.pettyCashAccountId,
    pettyCashAccountCode: initialEntry.pettyCashAccountCode,
    pettyCashAccountName: initialEntry.pettyCashAccountName,
...
type PettyCashVoucherEditorPageProps = {
  mode: "create" | "edit"
  entryId?: string
  initialEntry?: PettyCashVoucherRead | null
}

export function PettyCashVoucherEditorPage({
  mode,
  entryId,
  initialEntry = null,
  ...
  const [pettyCashAccountName, setPettyCashAccountName] = useState(seed.pettyCashAccountName)
  ...
  const applyEntry = useCallback((loaded: PettyCashVoucherRead) => {
    ...
    setPettyCashAccountName(loaded.pettyCashAccountName)
  ...
  useEffect(() => {
    void fetchGlAccounts({
      ...
      if (mode === "create" && !pettyCashAccountId) {
        const resolved = resolvePcvPettyCashAccount(result.accounts)
        if (resolved) {
          setPettyCashAccountId(resolved.id)
          setPettyCashAccountCode(resolved.code)
          setPettyCashAccountName(resolved.name)
        }
      }
    })
  }, [mode, pettyCashAccountId])
  ...
  async function ensurePettyCashResolved(): Promise<boolean> {
    if (pettyCashAccountId.trim()) return true
    setError("Petty cash account is required.")
    return false
  }

  async function handleSave(): Promise<PettyCashVoucherRead | null> {
    setError(null)
    ...
    if (!(await ensurePettyCashResolved())) {
      return null
    }
  ...
  async function runWorkflow(
    action: string,
    fn: () => Promise<PettyCashVoucherRead>
  ): Promise<void> {
    setError(null)
    setStatusMessage(null)
  ...
  const documentNo = formatPettyCashVoucherDocumentNo(entry?.entryNo)
  const entryRefNo = formatPcvEntryRefNo(entry?.entryNo)
    ...
    <div className="space-y-4" data-testid="petty-cash-voucher-editor">
      {(isPosted || isCancelled) && entry ? (
        ...
        <div
          ...
          data-testid="pcv-entry-shell"
        >
          <div
            className={`${financeAuditLine} flex flex-wrap items-baseline gap-x-4 gap-y-2 text-sm text-zinc-800`}
            data-testid="pcv-entry-meta-row-1"
          >
            <span className="shrink-0">
              Ref. No.:{" "}
              <span className="font-mono font-medium" data-testid="pcv-entry-ref-no">
                {entryRefNo}
              </span>
            </span>
            <span className="flex min-w-0 flex-wrap items-baseline gap-x-1 gap-y-1">
              ...
              {canEditHeader ? (
                ...
                <span data-testid="pcv-entry-date-prepared">
          ...
          <div
            className={`${financeAuditLine} pcv-entry-meta-row-2 text-sm text-zinc-800`}
            data-testid="pcv-entry-meta-row-2"
          >
            <div className="pcv-entry-meta-field pcv-entry-meta-petty-cash">
              <input
                readOnly
                tabIndex={-1}
                className="pcv-entry-meta-control pcv-entry-meta-locked rounded border border-zinc-300 px-2 py-0.5 text-sm"
                value={
                  pettyCashAccountId
                    ? pettyCashLockedLabel
                    : "Petty Cash account not resolved"
                }
                aria-readonly="true"
                data-testid="field-petty-cash-account"
              />
            </div>
            <div className="pcv-entry-meta-field pcv-entry-meta-cheque">
              <input
                disabled
            ...
            <div className="pcv-entry-meta-field pcv-entry-meta-payee">
              {canEditHeader ? (
                <input
                  className="pcv-entry-meta-control rounded border border-zinc-300 px-2 py-0.5 text-sm disabled:bg-zinc-50"
                  value={payeeName}
                ...
                <input
                  readOnly
                  tabIndex={-1}
                  className="pcv-entry-meta-control pcv-entry-meta-readonly rounded border border-zinc-300 px-2 py-0.5 text-sm"
                  value={payeeName || "—"}
                  data-testid="field-payee-name-readonly"
                />
              )}
            </div>
            <div className="pcv-entry-meta-field pcv-entry-meta-description">
              {canEditHeader ? (
                <input
                  className="pcv-entry-meta-control rounded border border-zinc-300 px-2 py-0.5 text-sm disabled:bg-zinc-50"
                  value={description}
                ...
                <input
                  ...
                  className="pcv-entry-meta-control pcv-entry-meta-readonly rounded border border-zinc-300 px-2 py-0.5 text-sm"
                  value={description || "—"}
                  data-testid="field-description-readonly"
                />
              )}
            </div>
          </div>
          ...
          {readOnly ? (
            <p className="text-sm text-zinc-600" data-testid="read-only-notice">
              ...
              {entry?.status
                ? formatPettyCashVoucherStatusLabel(entry.status)
                : status}
              .
            </p>
          ) : null}
          ...
          <div className={financeTableScroll}>
            <table
              ...
              data-testid="petty-cash-voucher-lines-table"
            >
              <colgroup>
          ...
          <div className="pcv-petty-cash-balance-box" data-testid="pcv-petty-cash-balance">
            <p className="font-medium">Petty Cash Balance</p>
            <table className="pcv-balance-table pav-entry-lines-table">
              <colgroup>
                <col className="pav-col-account" />
                <col className="pav-col-debit" />
                <col className="pav-col-credit" />
              ...
              <tbody>
                <tr className="pcv-balance-row" data-testid="pcv-balance-current-row">
                  <td className="pcv-balance-label">Current balance:</td>
                  <td />
                  <td className={financeNumber} data-testid="pcv-balance-current">
                    {formatAmount("0")}
                  </td>
                  <td />
                  {canEditLines ? <td /> : null}
                ...
                <tr className="pcv-balance-row" data-testid="pcv-balance-after-row">
                  <td className="pcv-balance-label">After voucher:</td>
                  <td />
                  <td className={financeNumber} data-testid="pcv-balance-after">
                    {formatAmount("0")}
                  </td>
                  <td />
                  {canEditLines ? <td /> : null}

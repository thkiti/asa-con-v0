...
import {
  ...
  type InvoiceVoucherListFilterInput,
...
function toListFilter(filter: FilterState): InvoiceVoucherListFilterInput {
  return {
    ...(filter.legalEntityCode
      ? { legalEntityCode: filter.legalEntityCode as DocumentEntityCode }
      : {}),
    ...(filter.status ? { status: filter.status as InvoiceVoucherStatusCode } : {}),
...
export function InvoiceVoucherListPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const result = await fetchInvoiceVouchers(toListFilter(filter))
  ...
  async function handleDelete(row: InvoiceVoucherListItem) {
    ...
    setError(null)
    try {
      await deleteDraftInvoiceVoucher(row.id)
      await load()
    } catch (err) {
  ...
  return (
    <div className="space-y-4" data-testid="invoice-voucher-list">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        <label className="flex flex-col gap-1 text-sm sm:col-span-2 lg:col-span-2">
          ...
          <input
            ...
            placeholder="INV no, customer, reference…"
            value={filter.search}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Link
          href="/finance/invoice-vouchers/new"
      ...
      {loading ? <p className="text-sm text-zinc-500">Loading invoice vouchers…</p> : null}
        ...
        <>
          ...
          <div className={financeTableScroll}>
            <table className={financeTable}>
              <thead>
                <tr>
                  <th className={financeTh}>INV no</th>
                  <th className={financeTh}>Invoice date</th>
                  <th className={financeTh}>Due date</th>
              ...
              <tbody>
                {entries.map((row) => (
                  <tr key={row.id}>
                    <td>
                      <Link
                        href={`/finance/invoice-vouchers/${row.id}`}
                    ...
                    <td className={financeMemo}>
                      {formatFinanceDocumentDate(row.invoiceDate)}
                    </td>
                    <td className={financeMemo}>
                      {row.dueDate ? formatFinanceDocumentDate(row.dueDate) : "—"}
                    </td>
                    <td className={financeMemo}>{row.customerName}</td>
